...
class Controller:
    ...
    def generateRoutes(self):
        ...
        match heuristik:
            ...
            case "Euclidean":
                self.heuristik = "euklid"
            case "Euclidean²":
                self.heuristik = "eklid_quadrat"
            case "Manhattan":
                self.heuristik = "manhattan"
        ...
        if len(self.routen_punkte) == 0:
            ...
            (
                ...
                start=self.start,
                target=self.end,
                metric=self.heuristik,
                weight=self.weight,
                abort=self.ui.checkbox_target.isChecked(),
            )
            ...
            if len(self.found_path) > 0:
                ...
                while i < len(keys):
                    print(next_start)
                    besuchte_routen, found_path, cost, length, travel_time = algo(
                        graph=self.graph,
                        start=next_start,
                        target=keys[i],
                        metric=self.heuristik,
        ...
        if len(self.found_path) > 0:
            ...
            print(
                f"Gewicht: {round(self.cost)}, Länge: {round(self.length)}m, Dauer: {round(self.travel_time)}s"
            )

            self.ui.label_cost_weight.setText(
                f"Gewicht: {round(self.cost)}, Länge: {round(self.length)}m, Dauer: {round(self.travel_time)}s"
            )
    ...
    def save_graphml(self, test=False, filepath=""):
        ...
        if filepath != "" and self.graph is not None:
            ...
            self.thread = QThread()
            self.worker = self.SaveGraphMLWorker(
                graph=self.graph,
                filepath=filepath,
                gephi=self.ui.checkbox_gephi.isChecked(),
            )
    ...
    class SaveGraphMLWorker(QObject):
        ...
        def run(self):
            ox.io.save_graphml(
                self.graph, filepath=self.filepath, gephi=self.gephi, encoding="utf-8"
            )
